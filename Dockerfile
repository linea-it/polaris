...
COPY --from=builder /src/app/build /var/www/monitor
...
COPY .env.template /var/www/monitor/.env
RUN chmod +x /var/www/monitor/env.sh
WORKDIR /var/www/monitor

# Start Nginx server recreating env-config.js
CMD ["/bin/bash", "-c", "/var/www/monitor/env.sh && nginx -g \"daemon off;\""]